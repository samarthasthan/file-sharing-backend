...
func init() {
	...
	ZIPKIN_PORT = env.GetEnv("ZIPKIN_PORT", "10411")
}

func NewTracer(serviceName string) (*zipkin.Tracer, reporter.Reporter, error) {

	// Create a reporter to be used by the tracer
	reporter := httpreporter.NewReporter(fmt.Sprintf("http://%s:%s/api/v2/spans", ZIPKIN_HOST, ZIPKIN_PORT))

	// Local endpoint represent the local service information
	localEndpoint := &model.Endpoint{ServiceName: serviceName, IPv4: getOutboundIP()}
